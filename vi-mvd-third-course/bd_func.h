/* создание таблицы дл€ авторизации*/
int create_table_auth(PGconn *conn){
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK){
        cout << "“аблица авторизации не создана: " << PQresultErrorMessage(res) << endl;
        PQclear(res);
        return 1;
    }
    PQclear(res);
...
int create_table_opoz(PGconn *conn){
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK){
        cout << "“аблица опознани€ не создана: " << PQresultErrorMessage(res) << endl;
        PQclear(res);
        return 1;
    }
    PQclear(res);
...
void create_admin(PGconn *conn){
    // пон€тно, что нужно использовать хэш от пользовательского ввода парол€... пока тест
    string insert = "insert into auth (login, pass, role) values ('admin', '12345', 'admin');";

    PGresult *res = NULL;
    res = PQexec(conn, insert.c_str());
    if (PQresultStatus(res) != PGRES_COMMAND_OK){
        std::cout << "ќшибка создание администратора: " << PQresultErrorMessage(res) << std::endl;
    }
    PQclear(res);
    printf("—оздана роль администратора!\n");
}
